...
const fromCSV = (csv, nAttrs) => {
  const lines = csv.split('\n'); // dividir data en lineas
  const headers = lines[0].split(','); // primera linea seran los header
  const result = [];

  for (let i = 1; i < lines.length; i++) {
    const values = lines[i].split(','); // se obtienen los valores de cada línea

    const obj = {};

    // Determinamos el número de atributos a agregar
      const limit = nAttrs ? 
        Math.min(nAttrs, headers.length) : 
        headers.length;

      // Rellenamos el objeto con los atributos indicados
      for (let j = 0; j < limit; j++) {
        obj[headers[j]] = values[j];
    }

    result.push(obj); // Añadimos el objeto al resultado
  }
...
console.log(fromCSV(data, 2)); 
console.log(fromCSV(data, 3)); 
console.log(fromCSV(data, 4)); 
console.log(fromCSV(data, 5))
...
const replaceAt = (arr, index, newElement) => {
    // Creamos un nuevo array utilizando slice para cortar antes y después del índice a reemplazar
    return [
      ...arr.slice(0, index), // Incluimos los elementos desde el inicio hasta antes del índice
      newElement,             // Reemplazamos el valor en la posición del índice
      ...arr.slice(index + 1)  // Incluimos los elementos desde después del índice hasta el final
    ];
  };
...
const winnerByYear = (arr, year) => {
	// Filtramos los objetos que pertenecen al año dado
  const filteredByYear = arr.filter(person => person.year === year);
  
  // Ordenamos el array filtrado por el campo `ranking` en orden descendente
  const sortedByRanking = filteredByYear.sort((a, b) => b.ranking - a.ranking);
  
  // Tomamos los tres primeros y retornamos solo los nombres
  return sortedByRanking.slice(0, 3).map(person => person.name);
};
...
const normalize = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    const { id, ...resto } = arr[i]; // Desestructuramos para separar la `id`
    result[id] = resto; // Asignamos el objeto sin `id` al nuevo objeto `result`
  }
...
const resultadoConArray = normalize(collection);
console.log(resultadoConArray);
...
const compact = (arg) => {
  if (Array.isArray(arg)) {
    // Si es un array, retornamos uno nuevo filtrando los elementos falsys
    return arg.filter(Boolean);
  } else if (typeof arg === 'object' && arg !== null) {
    // Si es un objeto, creamos un nuevo objeto con las propiedades que no sean falsys
    return Object.keys(arg).reduce((acc, key) => {
      if (arg[key]) { // Solo agregamos la propiedad si su valor no es falsy
        acc[key] = arg[key];
      }
      return acc;
    }, {});
  }
  // Si no es ni objeto ni array, simplemente lo retornamos
  return arg;
};
